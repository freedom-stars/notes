...
注意：我们只应该在性能测试的时候才在代码中引入pprof，在负载高的情况下 Profiling 才有意义。
...
<img src="pprof.assets/image-20200929141509814.png" alt="image-20200929141509814" style="zoom: 50%;" />
...
Type: cpu
Time: Sep 29, 2020 at 1:48pm (CST)
Duration: 20.19s, Total samples = 2.30mins (682.64%)
Entering interactive mode (type "help" for commands, "o" for options)
(pprof)  
```

我们可以在交互界面输入`top5`来查看程序中占用CPU前5位的函数：

```bash
(pprof) top5
Showing nodes accounting for 137.71s, 99.90% of 137.85s total
Dropped 31 nodes (cum <= 0.69s)
      flat  flat%   sum%        cum   cum%
    57.58s 41.77% 41.77%    105.51s 76.54%  runtime.selectnbrecv
    42.91s 31.13% 72.90%     44.84s 32.53%  runtime.chanrecv
    32.21s 23.37% 96.26%    137.76s 99.93%  main.logicCode
     5.01s  3.63% 99.90%      5.04s  3.66%  runtime.newstack
```
...
```shell
go run http_pprof.go
```
访问 `Ehttp://127.0.0.1:6060/debug/pprof/`
<img src="pprof.assets/image-20200929141509814.png" alt="image-20200929141509814" style="zoom: 50%;" />
...
```
(pprof) top 5
Showing nodes accounting for 411.41s, 99.92% of 411.74s total
Dropped 24 nodes (cum <= 2.06s)
      flat  flat%   sum%        cum   cum%
   178.71s 43.40% 43.40%    311.38s 75.63%  runtime.selectnbrecv
   118.71s 28.83% 72.23%    123.09s 29.90%  runtime.chanrecv
   100.06s 24.30% 96.54%    411.52s 99.95%  main.logicCode
    13.93s  3.38% 99.92%     13.95s  3.39%  runtime.newstack
```
...
```
Type: inuse_space
Time: Sep 29, 2020 at 2:34pm (CST)
No samples were found with the default sample value type.
Try "sample_index" command to analyze different sample values.
Entering interactive mode (type "help" for commands, "o" for options)
```
- inuse_space:  程序的常住内存占用情况
- alloc_objects：程序内存临时分配情况
...
![undefined](pprof.assets/007Djzqpgy1g8bgr7fcaej30ja0fcq4r.jpg)
...
[![undefined](pprof.assets/007Djzqpgy1g8bgri75hmj30xl0ldgsh.jpg)](http://ww1.sinaimg.cn/large/007Djzqpgy1g8bgri75hmj30xl0ldgsh.jpg)
